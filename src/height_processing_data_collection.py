...
def apply_sentinel1_month(polygon, year, month,directory_path, filename, scale=10):
    ...
    download_ee_image(ee.Image(image),os.path.join(directory_path,filename), scale=10, region=roi, crs="EPSG:4326")

    return ee.Image(image)



def apply_sentinel2_monthly(polygon, year, month,directory_path, filename,scale=10):
    def create_default_image(roi, band_names):
        default_bands = [ee.Image.constant(-1).rename(band).clip(roi) for band in band_names]
        default_image = ee.Image.cat(default_bands)
        return default_image
    ...
    dataset = dataset.map(scale_bands)
...
def add_nicfi_monthly(i, polygon, year, month,directory_path,filename,scale=10):
    ...
    nicfi = ee.ImageCollection('projects/planet-nicfi/assets/basemaps/africa') \
        .filterBounds(roi) \
        .filter(ee.Filter.date(f'{year}-{month:02d}-01', f'{year}-{month:02d}-28')) \
        .first()  # Get the first (and only) image for the month
...
def apply_landsat8_monthly(polygon, year, month, directory_path, filename, scale=10):
...
def dowload_one_polygon_for_one_month(id,polygon,year,month,directory_path_input,scale=10):
    os.makedirs(directory_path_input, exist_ok=True)
    try:
        apply_sentinel1_month(polygon,year,month,directory_path_input,f"{id}_S1_{year}_{month}.tif",scale=scale)
    except:
        print("no_sentinel1_data")
    try:
        apply_sentinel2_monthly(polygon,year,month,directory_path_input,f"{id}_S2_{year}_{month}.tif",scale=scale)
    except:
        print("no_sentinel2_data")
    try:
        add_nicfi_monthly(id,polygon,year,month,directory_path_input,f"{id}_nicfi_{year}_{month}.tif",scale=scale)
    except:
        print("no_nicfi_data")
    try:    
        apply_landsat8_monthly(polygon,year,month,directory_path_input,f"{id}_LC8_{year}_{month}.tif",scale=scale)
    except:
